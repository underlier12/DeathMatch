...
public class IndianController {
	...
	@ResponseBody
	@PostMapping
	public String creatRoom(@RequestBody String roomName) {
		IndianGameRoom indianRoom = indianRoomService.createRoom(roomName);
		String currentRoomId = indianRoom.getRoomId();
		log.info("currnetRoomId: " + currentRoomId);
		return "/indianHome/" + currentRoomId;
	}	

	
	@GetMapping("/{roomId}")
	 public String room(@PathVariable String roomId, Model model, HttpSession httpSession) {    	
		IndianGameRoom room = indianRoomService.findRoomById(roomId);
